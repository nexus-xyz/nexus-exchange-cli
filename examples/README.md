...
| --- | --- | --- |
| [`market_data.sh`](./market_data.sh) | Browse markets, tickers, book, trades, candles, funding, funding samples, mark price, status, risk params, ADL events, service status, venue stats | `GET /markets`, `/tickers`, `/markets/{id}/{ticker,orderbook,trades,candles,funding,mark-price,status,risk-params,adl-events}`, `/api/v1/markets/{id}/funding-samples`, `/markets/summary`, `/status`, `/stats`, `/stats/history` |
| [`account.sh`](./account.sh) | Inspect a funded account: balance, open and closed positions, fills, open orders and order history, withdrawals, equity history, funding payments, deposits, cancel-on-disconnect, ADL history, rate limit | `GET /account`, `/positions`, `/fills`, `/orders`, `/withdrawals`, `/funding`, `/deposits`, `/api/v1/positions/closed`, `/api/v1/orders/history`, `/api/v1/account/{equity-history,cancel-on-disconnect}`, `/account/{address}/adl-history`, `/account/rate-limit` |
| [`trading.sh`](./trading.sh) | Full order lifecycle: place, fetch, amend, batch, and every cancel variant (one, per-market, all) | `POST /orders`, `/orders/batch`, `GET /orders/{id}`, `PATCH /orders/{id}`, `DELETE /orders/{id}`, `DELETE /orders` |
...
| [`bridge.sh`](./bridge.sh) | Cross-chain deposits: list bridgeable assets, get a deposit address, watch a deposit confirm | `GET /api/v1/bridge/assets`, `GET/POST /api/v1/bridge/deposit-addresses`, `GET /api/v1/bridge/deposits`, `/deposits/{id}` |
...
| [`north_star.sh`](./north_star.sh) | **North star, unassisted:** faucet-fund → place a resting order → inspect → cancel, top to bottom against testnet with no editing | `POST /account/credit`, `GET /account`, `/markets/{id}/mark-price`, `POST /orders/batch`, `GET /orders`, `/orders/{id}`, `/positions`, `/fills`, `DELETE /orders/{id}` |
