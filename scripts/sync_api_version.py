#!/usr/bin/env python3
"""Advance the CLI's pinned spec version when nexus-exchange-api releases a newer
spec.

Ported from the same-named script in nexus-exchange-rs, the reference
implementation for this pipeline (ENG-3563 / ENG-7962). The CLI pins a released
spec version in `.api-version` and states it in a bot-managed README line. Nothing
advanced that pin when the api repo cut a release, so the CLI could fall behind
silently — the drift check only ever answers "do we still match the pin we have?".
The two scripts are complements:

  * check_spec_drift.py  — does the CLI still match the *pinned* spec? (drift)
  * sync_api_version.py  — has a *newer* spec released than we pin?     (lag)

What it does NOT do: touch the prose around the managed block, the coverage
numbers, or anything about the `nexus-exchange` dependency. Only `.api-version`
and the marked README line are bot-owned.

One CLI-specific note. The CLI is a thin layer over the published
`nexus-exchange` crate and issues no HTTP of its own, so a spec release that adds
operations cannot be *implemented* here until that crate ships wrappers for them.
The pin can still advance: the pin records which spec version the CLI targets and
sends as `X-Nexus-Api-Version`, and the drift check verifies only that the
operations the CLI already reaches still exist. Advancing the pin never obliges
the CLI to grow coverage.
...
def version_key(tag):
    # Pad to 3 components so (0, 7) and (0, 7, 0) compare equal.
    parts = parse_tag(tag)
...
def render_managed_block(new_tag):
    return (
        f"{MARK_START}\n\n"
...
def update_readme(new_tag):
    """Rewrite the managed block to point at new_tag. Returns True if README
    changed. Fails loudly if the markers are missing — they are added in the PR
    that introduces this workflow, so their absence is a setup error, not a silent
    no-op."""
    try:
    ...
    block = render_managed_block(new_tag)
