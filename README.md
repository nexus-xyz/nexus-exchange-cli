...
2. The [`release-please`](./.github/workflows/release-please.yml) workflow keeps a
   ...
   lockstep) and the changelog boundary the next version is computed from
   ([ENG-3921](https://linear.app/nexus-labs/issue/ENG-3921)), all of which hold
   at every version.
3. Merging that release PR creates the `v<version>` tag — the workflow pushes it
   itself, because a *draft* Release carries no git ref, and it re-creates the ref
   on any later push if it ever goes missing. That tag is the boundary
   release-please bounds the next changelog at, so a missing one makes it
   re-propose commits that already shipped.
...
which starts `release.yml` again. So deleting the tag alone re-runs the release;
that is the point when you *want* a retry, and the wrong outcome when you are
pulling one. To pull a release, do one of:

- **delete the draft Release too** — with nothing to recover the commit from, the
  workflow stops re-creating the tag; or
- **write `do-not-retag` anywhere in the draft Release's body** — the workflow
  then leaves it alone and says so.

Either way the boundary is now missing, so the workflow fails the job and skips
grooming the release PR rather than walking the whole history for a changelog.
Fix it by finishing the abort (drop the manifest version back) or by re-tagging
the release commit by hand.
...
Currently targets Exchange API spec **`v0.8.1`** — the version pinned and sent as `X-Nexus-Api-Version` by `nexus-exchange` **`0.11.1`**.
